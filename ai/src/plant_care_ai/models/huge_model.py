"""Model which combines genus and disease classification, supporting ResNet50 or EfficientNetV2 backbones.
...
class HugePlantModel(nn.Module):
    def __init__(
        self,
        ...
        *,
        ...
        # genus detection (freozen when 'leaves' are trained)
        self.head_species = nn.Linear(512,num_species)

        # leaves' disease
        self.head_health = nn.Linear(512, 1) # chance of being sick
        self.head_disease = nn.Linear(512, num_diseases) # name of the disease (if available)
    ...
    def freeze_species_head(self) -> None:
        for p in self.head_species.parameters():
            p.requires_grad = False
